...
import { catchError, concatMap, debounceTime, distinctUntilChanged, EMPTY, finalize, from, of, switchMap } from 'rxjs';
...
import { Gender } from '../model/client-gender';
...
@Component({
  ...
  styleUrl: './manage-client.scss',
})
export class ManageClient extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.sub$.sink = this.activeRoute.data.subscribe((data) => {
      ...
      if (clientData.client) {
        ...
        this.clientForm.get('email')?.disable(); // disable email field in edit mode
        // Initialize FormArray for policies as array of strings
  ...
  saveClient(isEmail?: boolean) {
    ...
    if (client.dateOfBirth && client.dateOfBirth instanceof Date) {
      client.dateOfBirth = (client.dateOfBirth as Date).toISOString();
    }
    if (client.commencementDate && client.commencementDate instanceof Date) {
      client.commencementDate = (client.commencementDate as Date).toISOString();
    }
