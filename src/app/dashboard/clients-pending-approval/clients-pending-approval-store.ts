...
export const ClientPendingApprovalStore = signalStore(
  { providedIn: 'root' },
      ...
      return ({
        loadByQuery: rxMethod<ClientResource>(
          pipe(
            ...
            switchMap((filter) => {
              return clientService.getClientsApprovalPending(filter).pipe(
                tapResponse({
                  next: (httpResponse) => {
                    const response = httpResponse as HttpResponse<Client[]>;
